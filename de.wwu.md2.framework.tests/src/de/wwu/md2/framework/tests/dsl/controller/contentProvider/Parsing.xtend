...
class Parsing {
	...
	@Test
	def simpleContentProviderTest(){
		var controllerModel = SIMPLE_CONTENTPROVIDER_C.load.parse(rs);
		controllerModel.assertNoErrors;		
	} 
	
	@Test
	def whereContentProviderTest(){
		var controllerModel = WHERE_CONTENTPROVIDER_C.load.parse(rs);
		controllerModel.assertNoErrors;		
	} 
	
	@Test
	def filteredContentProviderTest(){
		var controllerModel = FILTERED_CONTENTPROVIDER_C.load.parse(rs);
		controllerModel.assertNoErrors;		
	} 
}
