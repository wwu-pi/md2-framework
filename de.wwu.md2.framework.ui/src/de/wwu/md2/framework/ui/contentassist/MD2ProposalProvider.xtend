...
class MD2ProposalProvider extends AbstractMD2ProposalProvider {
	...
	override def completeMD2Model_Package(EObject model, Assignment assignment,
		...
		val proposal = "package " + util.getPackageNameFromPath(model.eResource().getURI());
		val display = new StyledString();
	...
	override def completePackageDefinition_PkgName(EObject model, Assignment assignment,
		...
		val proposal = util.getPackageNameFromPath(model.eResource().getURI());
	...
	override def complete_Boolean(EObject model, RuleCall ruleCall,
		...
		val displayStringTrue = new StyledString("true [boolean]");
	...
	override def completeKeyword(Keyword keyword, ContentAssistContext contentAssistContext, ICompletionProposalAcceptor acceptor) {
		...
		for (EObject eObject : contentAssistContext.getFirstSetGrammarElements()) {
			if(eObject instanceof RuleCall && (eObject as RuleCall).getRule().getName().equals("Boolean")
		...
		if(!keyword.getValue().equals("package") && contentAssistContext.getCurrentModel() instanceof MD2Model) {
			val model = contentAssistContext.getCurrentModel() as MD2Model;
			val parts = model.getPackage().getPkgName().split("\\.");
