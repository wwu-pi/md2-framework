...
class ProjectValidator extends AbstractMD2Validator {
	...
	def checkPackageNameCompliesWithLocation(MD2Model model) {
		...
		if (!model.getPackage().getPkgName().equals(pkgName)) {
			warning("The specified package does not match the actual location of the file",
				MD2Package.eINSTANCE.MD2Model_Package);
		}
	}
	...
	def checkModelElementCompliesWithPackage(MD2Model model) {
		...
		if (modelLayer instanceof Model && !lst.contains("models")) {
			error("You tried to put a model element in a non-model package",
				MD2Package.eINSTANCE.getMD2Model_ModelLayer, MODELELEMENT_PACKAGE);
		} else if (modelLayer instanceof View && !lst.contains("views")) {
			error("You tried to put a view element in a non-view package",
				MD2Package.eINSTANCE.getMD2Model_ModelLayer, MODELELEMENT_PACKAGE);
		} else if (modelLayer instanceof Controller && !lst.contains("controllers")) {
			error("You tried to put a controller element in a non-controller package",
				MD2Package.eINSTANCE.getMD2Model_ModelLayer, MODELELEMENT_PACKAGE);
		} else if (modelLayer instanceof Workflow && !lst.contains("workflows")) {
			error("You tried to put a workflow element in a non-workflow package",
				MD2Package.eINSTANCE.getMD2Model_ModelLayer, MODELELEMENT_PACKAGE);
		}
	}
}
