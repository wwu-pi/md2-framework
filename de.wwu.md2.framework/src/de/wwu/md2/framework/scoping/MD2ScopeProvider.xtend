...
class MD2ScopeProvider extends AbstractMD2ScopeProvider {
	...
	def IScope scope_FireEventEntry_event(FireEventEntry fireEventEntry, EReference eventRef) {
	...
	def IScope scope_PathTail_attributeRef(PathTail pathTail, EReference attributeRef) {
	...
	def IScope scope_EntityPathDefinition_entityRef(AbstractViewGUIElementRef context, EReference entityRef) {
		val resultSet = Sets.newHashSet();
		if (context.getRef() instanceof AutoGeneratedContentElement && !isRestrictedToContainer(context)) {
			for (ContentProviderReference ref : (context.getRef() as AutoGeneratedContentElement).getContentProvider()) {
	...
	def IScope scope_AbstractViewGUIElementRef_ref(EObject context, EReference ref) {
	...
	def IScope scope_AbstractViewGUIElementRef_ref(AbstractViewGUIElementRef context, EReference ref) {
