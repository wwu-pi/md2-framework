...
class TypeResolver {
	...
	static dispatch def MD2Type calculateType(Plus expr){
		if(expr.rightOperand?.calculateType === MD2Type.FLOAT)
			return MD2Type.FLOAT
			
		return expr.leftOperand?.calculateType
	}
	...
	static dispatch def MD2Type calculateType(Div expr){
		return expr.rightOperand?.calculateType
	}
	...
	static dispatch def MD2Type calculateType(ViewElementType elementType){	
		switch elementType {
			AutoGeneratedContentElement: return elementType.contentProvider.get(0)?.contentProvider?.type?.calculateType
