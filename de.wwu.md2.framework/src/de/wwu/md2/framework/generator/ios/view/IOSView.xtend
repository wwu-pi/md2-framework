...
class IOSView {
	...
	def static String generateGridLayoutPane(GridLayoutPane element, ContainerElement container) '''
		«val qualifiedName = WidgetMapping.fullPathForViewElement(element).toFirstLower»
		...
		«IF container != null»
		«WidgetMapping.fullPathForViewElement(container).toFirstLower».addWidget(«WidgetMapping.fullPathForViewElement(element).toFirstLower»)
		«ENDIF»
	...
	def static String generateSpacer(Spacer element, ContainerElement container) '''
		...
		«val qualifiedName = WidgetMapping.fullPathForViewElement(container).toFirstLower + "_Spacer" + MD2GeneratorUtil.getName(element)»
		...
		«««Add to surrounding container»»»
		«WidgetMapping.fullPathForViewElement(container).toFirstLower».addWidget(«qualifiedName»)
