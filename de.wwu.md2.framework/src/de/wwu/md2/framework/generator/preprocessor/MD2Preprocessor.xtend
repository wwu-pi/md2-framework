...
class MD2Preprocessor extends AbstractPreprocessor {
	...
	private def preprocessModel() {
		...
		val viewPreprocessor = new ProcessView
		...
		workflowElements.forEach[wfe | 
			...
			// all autogenerator elements (only within the respective wfe to avoid multiple calls!)
			val autoGenerators = wfe.processChain.flatMap[it.processChainSteps].map[it.view.ref].flatMap[it.eAllContents.filter(AutoGeneratedContentElement).toList].toList
		...
		viewPreprocessor.setFlowLayoutPaneDefaultParameters // done
		
		viewPreprocessor.duplicateSpacers // done
		
		viewPreprocessor.replaceNamedColorsWithHexColors // done
		...
		viewPreprocessor.transformInputsWithLabelsAndTooltipsToLayouts // done
		
		workflowElements.forEach[wfe |
			viewPreprocessor.createDisableActionsForAllDisabledViewElements(wfe) // done (with TODO)
		] 
		
		// Remove redundant elements
		val Collection<EObject> objectsToRemove = newHashSet
		objectsToRemove.addAll(view.eAllContents.toIterable.filter(AutoGeneratedContentElement))
		...
		viewPreprocessor.transformFlowLayoutsToGridLayouts // done 
		
		viewPreprocessor.calculateNumRowsAndNumColumnsParameters // done
		
		viewPreprocessor.fillUpGridLayoutsWithSpacers // done
		
		viewPreprocessor.calculateAllViewElementWidths // done
