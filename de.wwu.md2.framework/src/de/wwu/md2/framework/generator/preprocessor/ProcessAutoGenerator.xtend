...
class ProcessAutoGenerator extends AbstractPreprocessor {
	 ...
	 */
	def createViewElementsForAutoGeneratorAction(Collection<AutoGeneratedContentElement> autoGenerators, WorkflowElement wfe) {
		...
		autoGenerators.forEach [ autoGenerator | 
			...
			while (!(parentContainer instanceof ContentContainer) && !(parentContainer instanceof ViewFrame)) {
				parentContainer = parentContainer.eContainer
			}
			
			val elements = if(parentContainer instanceof ContentContainer) {
							parentContainer.elements
						} else if(parentContainer instanceof ViewFrame) {
							parentContainer.elements
						} 
			...
			autoGenerator.contentProvider.forEach [ contentProviderReference |
				...
				switch (contentProvider.type) {
					ReferencedModelType: {
						...
						val viewDefs = modelToView(modelElement, pathDefinition, filteredAttributes, wfe, autoGenerator).toList						
...
//						}					
	...
	def private modelToView(
		ModelElement m, ContentProviderPath pathDefinition,	Collection<EntityPath> filteredAttributes, WorkflowElement wfe, AutoGeneratedContentElement autoGenerator
	) {
		modelToView(m, pathDefinition, filteredAttributes, null, wfe, autoGenerator)
	}
	
	def private Iterable<ViewGUIElement> modelToView(
		ModelElement m, ContentProviderPath pathDefinition, Collection<EntityPath> filteredAttributes, String labelPrefix, WorkflowElement wfe, AutoGeneratedContentElement autoGenerator
	) {
...
//					} else {
						switch (a.type) {
							ReferencedType: {
								if ((a.type as ReferencedType).element instanceof Entity) {
									...
									for (childElem : modelToView((a.type as ReferencedType).element, copyElement(pathDefinitionIter), filteredAttributes, a.labelText + " - ", wfe, autoGenerator)) {
						...
						// Replace abstractviewGuiElementRef references with path, e.g. MainView.bookInfoPart[Book.isbn]
						val refs = EcoreUtil2.getRootContainer(wfe).eAllContents.filter(AbstractViewGUIElementRef).filter[it.path !== null].toList
						for(ref : refs){
							var attribute = ref.path.tail
							while(attribute.tail !== null){
								attribute = attribute.tail
							}
							if(attribute.attributeRef === a){
								ref.ref = guiElementToAdd
								ref.path = null
							}
						}
//					}
