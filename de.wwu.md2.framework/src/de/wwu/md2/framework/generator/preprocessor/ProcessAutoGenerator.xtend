...
class ProcessAutoGenerator extends AbstractPreprocessor {
	 ...
	 * </ul>
	 */
	def createAutoGenerationAction(Iterable<AutoGeneratedContentElement> autoGenerators) {
		...
		if (getAutoGenAction == null) {
			val ctrl = controllers.last
			
			if (ctrl != null) {
				
				val startupAction = controllers.map[ c |
					c.controllerElements.filter(typeof(CustomAction))
	 ...
	 */
	def createViewElementsForAutoGeneratorAction(Iterable<AutoGeneratedContentElement> autoGenerators) {
			...
			autoGenerator.contentProvider.forEach [ contentProviderReference |
				...
				switch (contentProvider.type) {
					ReferencedModelType: {
						...
						val viewDefs = modelToView(modelElement, pathDefinition, filteredAttributes).toList						
...
//						}					
					...
					SimpleType: {
						...
						val viewDef = modelToView2(simpleDataType, pathDefinition)
	...
	def private modelToView(
		ModelElement m, ContentProviderPath pathDefinition,	Collection<EntityPath> filteredAttributes
	) {
		modelToView(m, pathDefinition, filteredAttributes, null)
	}
	
	def private Iterable<ViewGUIElement> modelToView(
		ModelElement m, ContentProviderPath pathDefinition, Collection<EntityPath> filteredAttributes, String labelPrefix
	) {
...
//					} else {
						...
						addDataMapping(guiElementToAdd, pathDefinitionIter)				
//					}
	...
	def private modelToView2 (SimpleDataType s, ContentProviderPath pathDefinition) {
		...
		addDataMapping(viewElement, pathDefinition)
		autoGenerationCounter = autoGenerationCounter + 1
		viewElement
	}
	
	def private addDataMapping(ViewGUIElement guiElem, ContentProviderPath pathDefinition) {
		val autoGenAction = getAutoGenAction
